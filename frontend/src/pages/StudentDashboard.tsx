...
export const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const response = await apiService.getStudentAttempts(user.token);
        const attempts = response.data.length;
        const solved = response.data.filter(
          (a: Attempt) => a.status === "CORRECT"
        ).length;
        setStats({ attempts, solved });
      } catch (err) {
        const axiosError = err as AxiosError<{ detail?: string }>;
        console.error("Failed to fetch stats:", axiosError);
      }
    };
    fetchStats();
  }, [user]);
  ...
  const data: ChartData<"pie", number[], string> | null = stats
    ? {
        labels: ["Попытки", "Решено"],
        datasets: [
          {
            data: [stats.attempts, stats.solved],
          ...
          <div className="flex-1">
            {editMode ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-600">
                  <span className="font-medium">Роль:</span> Ученик
                </p>
              </div>
            )}
          </div>
